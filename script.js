...
// Function to render the Pie Chart for expense distribution
function renderExpenseChart() {
    ...
    group.expenses.forEach(expense => {
        if (expense.payer in expenseData) {
            expenseData[expense.payer] += expense.amount;
        } else {
            expenseData[expense.payer] = expense.amount;
        }
    });
...
// Function to render the Bar Chart for monthly spending trends
function renderMonthlyChart() {
    ...
    new Chart(ctx, {
        type: "bar",
        data: {
            labels: Object.keys(monthlyData).map(m => `Month ${parseInt(m) + 1}`),
            datasets: [{
